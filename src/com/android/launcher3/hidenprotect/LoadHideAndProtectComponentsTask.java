...
public class LoadHideAndProtectComponentsTask extends AsyncTask<Void, Integer, List<HideAndProtectComponent>> {
    @NonNull
    ...
    LoadHideAndProtectComponentsTask(@NonNull HideAndProtectDatabaseHelper dbHelper,
    ...
    @Override
    protected List<HideAndProtectComponent> doInBackground(Void... voids) {
        List<HideAndProtectComponent> list = new ArrayList<>();
        ...
        for (int i = 0; i < numPackages; i++) {
            ...
            try {
                ...
                list.add(new HideAndProtectComponent(pkgName, icon, label, isHidden, isProtected));
    ...
    @Override
    protected void onPostExecute(List<HideAndProtectComponent> HideAndProtectComponents) {
        mCallback.onLoadCompleted(HideAndProtectComponents);
    }

    interface Callback {
        void onLoadListProgress(int progress);
        void onLoadCompleted(List<HideAndProtectComponent> result);
    }
}
